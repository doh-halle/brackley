defmodule Brackley.AccountTest do
  ...
  describe "reviews" do
    ...
    test "list_reviews/0 returns all reviews" do
      ...
      assert Account.list_reviews() == [review]
    end

    test "get_review!/1 returns the review with given id" do
      review = review_fixture()
      assert Account.get_review!(review.id) == review
    end

    test "create_review/1 with valid data creates a review" do
      valid_attrs = %{status: :pending, comments: 42, review: "some review", rating: 42}

      assert {:ok, %Review{} = review} = Account.create_review(valid_attrs)
      assert review.status == :pending
      assert review.comments == 42
      assert review.review == "some review"
      assert review.rating == 42
    end

    test "create_review/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Account.create_review(@invalid_attrs)
    end
    ...
    test "update_review/2 with valid data updates the review" do
      ...
      assert {:ok, %Review{} = review} = Account.update_review(review, update_attrs)
      assert review.status == :approved
      assert review.comments == 43
    ...
    test "update_review/2 with invalid data returns error changeset" do
      review = review_fixture()
      assert {:error, %Ecto.Changeset{}} = Account.update_review(review, @invalid_attrs)
      assert review == Account.get_review!(review.id)
    end

    test "delete_review/1 deletes the review" do
      review = review_fixture()
      assert {:ok, %Review{}} = Account.delete_review(review)
      assert_raise Ecto.NoResultsError, fn -> Account.get_review!(review.id) end
    end

    test "change_review/1 returns a review changeset" do
      review = review_fixture()
      assert %Ecto.Changeset{} = Account.change_review(review)
    end
  end
end
